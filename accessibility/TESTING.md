...
原則：**每新增或改動一個公開頁，就把下面六項跑一次**，把結果記到 audit.md。新頁要先測過再公開。

## 不在這裡測的：Google 表單

投稿用的 Google Form 是 Google 託管的，不是我們的 HTML。我們拿不到它的標記、跑不了上面那些工具、也改不到它，所以它不在這套流程裡。

**現況（查過）：** Google Forms 大致能用報讀器和鍵盤操作，Google 也有出無障礙符合度報告（VPAT），但**不是完全符合 WCAG，有已知差距**：日期題型對報讀器不友善、部分選項缺正確標籤、下拉選單與檔案上傳的焦點處理不一致、placeholder 對比偏低、拖放排序不能用鍵盤。

**好消息是我們的表單避開了最糟的那幾個**：只用了文字、下拉、核取、段落這些較穩的題型，沒有日期題、檔案上傳、格狀題。剩下可能沾到的（下拉焦點、核取標籤、對比）是 Google 的實作，我們改不了。一句話：**還算可用，但有我們無法修的已知瑕疵，靠 Google 自己改善。** 參考：Google 無障礙符合度報告 https://www.google.com/accessibility/
...
**版面（RWD，響應式）：** 用真的手機開頁面（或 DevTools 的裝置模式）：
- 直式和橫式都看，沒有左右橫向捲動、沒有東西被切掉。
- 文字不用放大就讀得到，按鈕夠大好點（手指按得到，不會誤觸旁邊的）。

**手機報讀器：** 手機也有內建報讀器，開起來走一遍：
- **iPhone（VoiceOver）：** 設定 → 輔助使用 → VoiceOver 打開（或連按三下側邊鍵）。**左右滑**移到上一個或下一個元素，**點兩下**啟動。
- **Android（TalkBack）：** 設定 → 協助工具 → TalkBack 打開。手勢類似（左右滑移動、點兩下啟動）。

檢查重點跟桌機報讀器一樣：**讀的順序合理、標題和地標跳得到、每個控制項都唸出清楚名稱**（切換鈕還要唸出狀態）。
